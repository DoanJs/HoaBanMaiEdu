...
export default function AdminChildren() {
  ...
  return (
    <RowComponent styles={{
      ...
      <div style={{ flex: 1, height: '100%', overflowY: 'scroll', padding: 16 }}>
        <RowComponent justify="space-between">
          <SearchComponent title="Tìm trẻ" placeholder="Nhập tên trẻ" width={'75%'}
            arrSource={children} type="searchChildren" onChange={(val) => setNewChildren(val)} />
          <TextComponent text={`Có ${newChildren.length} trẻ`} styles={{ fontWeight: 'bold' }} />
        </RowComponent>
          ...
          <tbody>
            {
              ...
              newChildren.map((child, index) =>
                <AdminChildComponent key={index} child={child} teachers={teachers} setChildEdit={setChildEdit} />)
            }
      ...
      <div style={{ flex: 1, background: colors.primaryLight, padding: '20px 40px', borderRadius: 10 }}>
        <RowComponent justify="space-evenly">
          <TextComponent text={childEdit ? 'Chỉnh sửa thông tin trẻ' : "Thêm trẻ mới"} size={sizes.title} styles={{ fontWeight: 'bold' }} />
          {childEdit && <RowComponent styles={{ cursor: 'pointer' }}>
            ...
            <TextComponent text="Xóa trẻ" size={sizes.bigText} />
          </RowComponent>}
        </RowComponent>
        ...
        <div>
          ...
          <input
            onChange={(val) =>
              setForm({ ...form, avatar: val.target.value })
            }
            type="fullName"
            className="form-control"
            value={form.avatar}
        ...
        >
          {isLoading ? <SpinnerComponent /> : <>{childEdit ? 'Cập nhật' : 'Đăng ký'}</>}
        </button>
      </div>
      ...
      data={{ id: childEdit?.id as string, 
        nameCollect: 'children', 
        itemTasks: []
      }} 
